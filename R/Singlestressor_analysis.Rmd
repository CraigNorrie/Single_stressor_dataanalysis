...
```{r}
pooled_site_survplot_DO_pooled <- survival_eventdat_DO %>% 
 ggsurvplot_facet(survfit(Surv(Time, event, type = "right") ~ Colour , data = .),
            facet.by = "DO",
            data = .,
            risk.table = FALSE, pval = FALSE, conf.int = TRUE, 
...
DO_resp_summarytable <- DO_respdat %>% filter(masscorrectedrate != "NA") %>% 
  ...
  summarise(n=n(), mean_rate=mean(masscorrectedrate), SEMrate=sd(masscorrectedrate)/sqrt(n()),
            mean_rate_gramgram=mean(O2gramgram), SEMgramgram=sd(O2gramgram)/sqrt(n()))
```

Create physiological performance curves. Start for the non heat shocked individuals. For these analyses and plots I was unable to achieve 100% oxygen saturation so I am removing the 100% treatment from analyses.

```{r}
DO_resp_summarytable %>% filter(Treatment != "80HS" & Treatment != "60HS" & Treatment !="20HS") %>% 
  filter(Treatment != "100") %>% 
  ggplot(aes(x=Treatment, y= mean_rate, group = Ploidy, colour = Ploidy))+geom_line(size = 1)+#base graph
...
Physiological curves for heat shocked individuals. 
...
The data was left skewed so a log transformation was performed
```{r}
hist(log(DO_respdat$masscorrectedrate))
```

The logged data has a much more normal distribution so this will be used in the further analyses. 

I don't have the DF to include the HS individuals within the overall test. Will need to perform a seperate test to see if the O2 consumption on all of these is higher. Can I include an additional HS column with only 2 variables? I think so. 

```{r}
DOresp_lme <- DO_respdat %>% filter(Treatment != "80HS" & Treatment != "60HS" & Treatment !="20HS") %>% 
  lmer(log(masscorrectedrate)~Ploidy*Treatment + (1|Tank), data=.)
summary(DOresp_lme)
```

Print the contrasts in oxygen consumption. CHECK THIS CAREFULLY (16 JAN 2024)
```{r}
emmeans_DOresp_lme <- emmeans(DOresp_lme, ~ Ploidy+Treatment)
DO_resp_contrasts <- contrast(emmeans_DOresp_lme)
DO_resp_contrastsdf <-  as.data.frame(DO_resp_contrasts)
print(DO_resp_contrastsdf %>% filter(p.value <=0.05))
```
