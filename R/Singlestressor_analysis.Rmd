...
DO_respdat <- read.csv(here("Data", "DO", "DO_resprates.csv")) %>% 
  select(Tank, Ploidy, Treatment, Oyster.tissue.dry.mass, masscorrectedrate) %>% #just select the relevant columns
...
DO_resp_summarytable %>% filter(Treatment != "80HS" & Treatment != "60HS" & Treatment !="20HS") %>% 
  ...
  ggplot(aes(x=Treatment, y= mean_rate, group = Ploidy, colour = Ploidy))+geom_line()+
  geom_errorbar(aes(ymin=mean_rate-SEMrate, ymax=mean_rate+SEMrate))
