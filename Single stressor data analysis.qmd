...
Individuals were held for a period of 4 weeks under these conditions. This is a supplementary file to Norrie et al. (in prep) single stressor diploid and triploid research mansucript. This markdown file anlysed differences in growth, respiration, and mortality 
...
The smaller shell height at the conclusion of the experiment was likely due to parts of the shell being chipped off or inaccuracies in measurements. Thus we do not include measures of shell height growth in the manuscript <Br>
...
As all individuals had been growing in a common environment prior to the start of the experiment we first compared the size of individuals at the start of the experiment using a linear model. <Br>

This model indicated that there was a significant difference in the mean mass of induced triploids between the 22.5 and 7.5 °C treatments. Therefore we did not simply compare the size of individuals at the end of the experiment for each ploidy across temperature treatments. <Br>

Differences in mean size of each ploidy existed at the start of the experiment. Diploids were the smallest and mated triploids were largest (@fig-temp_mass).This is what explains the significant treatment:ploidy interaction. <Br>
...
***In mated triploids*** we observed a significant main effect of time, but no interaction effects or treatment main effects (@lme_mass_dips_temp_3M). 
...
Again we copmared the mean size of each ploidy seperatley at the start and end of each experiment. We did not conmpare across ploidies as there are inherent differences in the size and growth of each ploidy. Mated triploids were larger than induced triploids and diploids at the start of this experiment. There was no difference in the size of diploids and mated triploids throughout this experiment. <Br>
...
pairs_ploidy <- pairs(pairwise_K5_pH, simple = "CO2_level", adjust = "Tukey")
pairs_CO2 <- pairs(pairwise_K5_pH, simple = "Colour", adjust = "Tukey")
```
