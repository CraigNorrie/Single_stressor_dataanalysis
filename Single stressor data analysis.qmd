...
ggforest(K4_pH)#visualise the data
```

In oprder to account for variation in mortality between tanks we performed a frailty analysis. This frailty analysis was akin to using a linear mixed model to analyse the data. 
...
Because there was a significant effect of mortality I need to do pairwise comparisons to understand exactly which treatments differ from one another. To do this I will need to re level the data so that everything is compared against everything else as the base level. a for loop will do this for all the data 
...
#                                                     distribution = emfrail_dist(dist = "gamma"), data = .) %>%
#   {.}
#This for loop may change the order automatically and save yje result with the name pattern
